...
_SYSTEM = (
    ...
    "clause of the resolution note it contradicts. The evidence package "
    "is COMPLETE: what it does not mention does not exist in this world "
    "-- absence of a usage or loss process means none occurs, and that "
    "is completeness, not silent resolution. The runtime enforces stock "
    "physics itself -- a transfer cannot move stock that is not there, "
    "and processes accrue on real calendars -- so do not demand explicit "
    "checker or comparison mechanisms for what the physics already "
    "enforces. A rate or duration carried as a labeled, cited average is "
    "the honest deterministic treatment (runtime numbers carry their "
    "provenance; they are not claims of zero variance). An uncertainty "
    "DECLARATION never changes the produced number -- a missing "
    "declaration is material only where it could flip an unresolved "
    "outcome into a definite one. Weekday numbers use Monday=0 through "
    "Sunday=6; renderings spell the day names -- trust them. Where "
    "several processes accrue into ONE stock, they must be genuinely "
    "distinct mechanisms with non-overlapping claims on the same hours; "
    "one real mechanism split into overlapping windows double-counts, "
    "and that IS a material defect of the causal spine. Never demand "
    "that an actor's action be scheduled or time-boxed: whether and "
    "when an actor acts is the simulation's own outcome, produced by "
    "their prerequisites, attention cadences, stated durations and "
    "simulated choices -- judge whether the capabilities and "
    "information paths exist, not when they will be used.\n"
    ...
    "There is ONE revision round: list EVERY material defect in this "
    "verdict -- a defect you notice but withhold can never be repaired. "
    "Before deciding, trace each scheduled movement end to end, each "
    "occurrence separately: a dispatch whose delivery never reaches the "
    "stock it feeds is a material gap NOW, not later. "
