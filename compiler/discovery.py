...
def discover(question: dict, evidence: dict, call=call_json,
    ...
    disc.spine = _ask(
        ...
        "open\n"
        "  basis, evidence_ids\n"
        "When the terminal counts records of individual acts (votes "
        "cast, approvals signed, filings made), EVERY such act step "
        "declares produces_proof: [that record's proof name] -- the act "
        "is exactly what makes its record; there is no separate "
        "recording machinery to invent.\n"
        ...
        "wrapper process around its arrival. Loading, carrying or "
        "handing over stock is never a producing process: relocation is "
        "the transfer itself, carried by the movement's event; a process "
        "only ever creates or consumes stock where it stands.\n"
        "KIND 'process' IS ONLY FOR ACCUMULATING OR CONSUMING A "
        "QUANTITY at a rate. A recurring mechanism that does something "
        "NON-quantitative -- a batch job that runs each weekday "
        "evening and releases what has cleared, a nightly sweep, a "
        "weekly posting run -- is NOT a process: write one "
        "scheduled_event per occurrence that falls between the world's "
        "opening and the deadline, each with its own 'when'. And a "
        "purely calendrical calculation -- counting business days to a "
        "statutory deadline, adding a notice period -- is not a "
        "mechanism at all: no step performs it. Work the date out "
        "yourself and put the resulting instant on the step that "
        "actually happens then.\n"
        "NEVER DECIDE AN OUTCOME HERE. Do not mark a step unsupported "
        "because you have worked out that the actor will run out of "
        "time, will arrive too late, or will not get there before some "
        "cutoff. Whether their slot falls before or after a closing is "
        "precisely what the simulation exists to determine. List what "
        "each actor CAN do and what it requires; the run decides what "
        "actually happens. A stated "
        "conditional -- 'X will do A only if Y first does B' -- is B's "
        "outcome listed as a necessary prerequisite of X's action A, "
        "with B's own step producing that outcome; never a floating "
        "condition nothing produces. Model "
        "each occurrence separately and completely: a Tuesday shipment "
        "and a Thursday shipment are two events.\n"
        "ANYTHING THAT TRAVELS NEEDS BOTH ENDS. Whenever something "
        "crosses distance over time -- goods shipped, a person flying "
        "or driving, a document couriered, a signature carried between "
        "offices -- the world needs BOTH a departure step and a "
        "SEPARATE arrival step, and it is the ARRIVAL that produces the "
        "effect at the destination: the stock credited, the person "
        "present, the document in the recipient's hands. Give the "
        "arrival its own 'when' if the evidence states one, otherwise "
        "anchor it to the departure and let the transit time set the "
        "offset. A departure with no arrival delivers nothing and "
        "nobody: whatever waits on the far end can then never happen. "
        "Equally, an arrival with no departure has nothing to anchor "
        "it. This holds for every category, not just stock.\n"
    ...
    producers_system = (
        ...
        "composed of, or something here must produce it. A measured "
        "record that act steps already produce (each cast vote declares "
        "produces_proof) needs NO entry here at all -- omit it; never "
        "mark it unsupported and never invent recording machinery for "
        "it. But the step "
    ...
    def discover_entity(ent, ecall=None, emodel=None):
        ...
        shape = (
            ...
            + ("  process_behavior: {\"meaning\": prose, \"rate_meaning\": "
               ...
               "  availability: {\"workdays\": [Monday=0, Tuesday=1, "
               "Wednesday=2, Thursday=3, Friday=4, Saturday=5, "
               "Sunday=6 -- an ordinary Monday-to-Friday week is "
               "[0,1,2,3,4], never [1,2,3,4,5]],"
