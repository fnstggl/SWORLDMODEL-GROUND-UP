"""Shared setup for the Phase 6 counterfactual-manager suite.
...
- Dummy LLM credentials are installed via ``os.environ.setdefault``
  ...
  module in this suite performs network I/O, but a transitive import must
...
- ``tests/engine_contracts`` and ``tests/engine_baseline`` are inserted
  ...
  seeded-determinism harness the Phase 2 contracts proved and the SAME
  ``baseline_helpers`` scripted-model toolkit the Phase 4 baseline proved
  (single source of truth; no copies to drift).

Run with the pinned engine environment:

    /home/user/engine-env/bin/python -m pytest tests/engine_counterfactuals -q
"""
